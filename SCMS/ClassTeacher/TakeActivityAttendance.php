...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
    ...
    try {
        // Fetch the activity mark
        $qry = "SELECT ActivityMark, ClubID FROM activity WHERE ActivityID = ?";
        ...
        for ($i = 0; $i < $totalRecord; $i++) {
            ...
            $query = "SELECT AttendanceID, AttendanceStatus FROM attendance WHERE ActivityID = ? AND StudentID = ?";
            ...
            if ($existingRecord) {
                ...
                if ($attendanceStatus === 'Present' && $previousStatus !== 'Present') {
                    ...
                    $stmt->bind_param("ssii", $currentDateTime, $attendanceStatus, $ActivityID, $studentID);
                    $stmt->execute();
                    ...
                    $stmt = $conn->prepare($updateQuery);
                    $stmt->bind_param("iii", $activityMark, $studentID, $clubID);
                    $stmt->execute();
                ...
                // Insert new record
                $insertQuery = "INSERT INTO attendance (ActivityID, StudentID, DateTimeTaken, AttendanceStatus)
                                VALUES (?, ?, ?, ?)";
                $stmt = $conn->prepare($insertQuery);
                $stmt->bind_param("iiss", $ActivityID, $studentID, $currentDateTime, $attendanceStatus);
                $stmt->execute();
                  ...
                  <div class="col-xl-6">
                    ...
                    <select required name="ActivityID" class="form-control mb-3">
                      ...
                      while ($row = mysqli_fetch_assoc($result)){
                        $aid = $row['ActivityID'];
                      ?>
                        <option value="<?php echo $aid ?>" <?php if($aid==$ActivityID) { echo "selected"; } ?> ><?php echo $row['ActivityName'] ?></option>
                      <?php
                      }
                      ?>
                    </select>
                ...
                <form method="post">
                  ...
                  <div class="card mb-4">
                    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                      <h6 class="m-0 font-weight-bold text-primary">All Students in Activity</h6>
                      ...
                      <table class="table align-items-center table-flush table-hover">
                        ...
                        <tbody>
                          ...
                          $query = "SELECT DISTINCT
                                    ...
                                    LEFT JOIN 
                                    attendance ON registerstudent.StudentID = attendance.StudentID AND attendance.ActivityID = ?
                                    INNER JOIN 
                                    ...
                                    AND activity.ActivityID = ? AND registerstudent.CurrentYear = '$CurrentYear'";
                          
                          // Prepare and execute the query
                          if ($stmt = $conn->prepare($query)) {
                            $stmt->bind_param("iii", $ActivityID, $_SESSION['userId'], $ActivityID);
                            $stmt->execute();
                            $result = $stmt->get_result();
                            $num = $result->num_rows;
                            $sn = 0;
